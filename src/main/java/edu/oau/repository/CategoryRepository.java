...
public interface CategoryRepository extends ReactiveCrudRepository<Category, Long>, CategoryRepositoryInternal {
    ...
    @Query("SELECT * FROM category entity WHERE entity.category_id = :id")
    Flux<Category> findByCategory(Long id);

    @Query("SELECT * FROM category entity WHERE entity.category_id IS NULL")
    Flux<Category> findAllWhereCategoryIsNull();
